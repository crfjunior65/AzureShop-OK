...
## 49. Atualização — estado confirmado e novo nome disponível

O estado Terraform confirmou que a zona foi registrada corretamente:
...
```text
Nome:           privatelink.database.windows.net
Resource Group: rg-imersao-arquitetoazure-us
Registros:      1, correspondente inicialmente ao SOA da zona
Gerenciado por: Terraform
```

A consulta do ACR `acrimersaoazureshop` na assinatura não retornou resultados. Isso confirma que o registro conflitante não é um ACR visível nessa assinatura. Não há recurso acessível para importar neste estado; o caminho adequado é usar outro nome global.

O nome candidato foi consultado:

```text
acrimersaoazureshop1965
```
...
A disponibilidade é válida no momento da consulta e pode mudar se outro cliente registrar o mesmo nome antes da aplicação.

## 50. O vínculo DNS–VNet ainda não está no estado

A lista atual não contém um recurso semelhante a:

```text
azurerm_private_dns_zone_virtual_network_link.aks
```

Portanto, o próximo plano deve ser revisado para confirmar a criação desse vínculo. Sem ele, máquinas e workloads na VNet não terão a associação necessária para usar essa Private DNS Zone.

Confirme que o bloco existe no código:

```bash
rg -n -C 8 'resource "azurerm_private_dns_zone_virtual_network_link"' .
```

Confirme também a referência corrigida:

```hcl
private_dns_zone_name = azurerm_private_dns_zone.portal_sql.name
```

## 51. Alterar somente o nome do ACR

Antes de editar, localize como o nome é calculado:

```bash
rg -n -C 8 'module "container_registry"|acrimersaoazureshop|registry_name|acr_name' .
```

E examine o módulo:

```bash
nl -ba modules/container-registry/main.tf | sed -n '1,35p'
nl -ba modules/container-registry/variables.tf | sed -n '1,120p'
```
...
O local correto da alteração depende das variáveis reais do módulo.

Evite alterar uma variável genérica como `suffix` sem verificar seus consumidores:

```bash
rg -n 'var\.suffix|suffix\s*=' .
```

Se `suffix` também nomear AKS, Key Vault, Storage, Private Endpoint ou outros componentes, modificá-lo pode provocar renomeações ou substituições indesejadas. Prefira uma variável exclusiva do ACR, como `acr_name` ou `container_registry_name`, se já existir. Se não existir, considere adicioná-la ao módulo em vez de reutilizar um sufixo global.

## 52. Revalidar a disponibilidade imediatamente antes do plano
...
Continue somente se `Available` permanecer `True`.

## 53. Gerar um plano após a aplicação parcial

Renomeie o plano salvo antigo, se ainda existir, para evitar sua aplicação acidental e preservar uma cópia temporária:

```bash
if [ -f tfplan ]; then mv tfplan tfplan-anterior; fi
```
...
O plano esperado deve apresentar:

- `azurerm_private_dns_zone.portal_sql`: sem alteração;
- `module.container_registry[0].azurerm_container_registry.this`: criação com o nome `acrimersaoazureshop1965`;
- `azurerm_private_dns_zone_virtual_network_link.aks`: criação, caso o bloco esteja habilitado;
- zero recursos destruídos.

Se houver substituição ou destruição de recursos existentes, pare e revise a variável alterada.

## 54. Aplicação e verificação

Somente após revisar o plano:
...
az acr show \
  --subscription "31f0817a-9efc-46d2-a0c2-3c9d85e77619" \
  --name "acrimersaoazureshop1965" \
  --query '{Name:name, LoginServer:loginServer, ResourceGroup:resourceGroup, Location:location, Sku:sku.name}' \
  --output table
```

Confirme o vínculo da zona:
...
az network private-dns link vnet list \
  ...
  --resource-group "rg-imersao-arquitetoazure-us" \
  --zone-name "privatelink.database.windows.net" \
  --query '[].{Name:name, VNet:virtualNetwork.id, RegistrationEnabled:registrationEnabled, State:virtualNetworkLinkState}' \
  --output table
```

## 55. Atualização — o novo nome não foi gravado no Terraform

O plano mais recente continua mostrando:

```hcl
name = "acrimersaoazureshop"
```

Portanto, o arquivo `tfplan-novo` ainda contém o nome indisponível e não deve ser aplicado.

As linhas abaixo foram digitadas diretamente no Bash:
...
Por isso o shell retornou:

```text
bash: name: command not found
bash: acr_name: command not found
```

Essas linhas são configuração HCL do Terraform, não comandos de terminal. Elas precisam ser escritas dentro de um arquivo `.tf` ou `.tfvars`.

## 56. Origem exata do nome do ACR

O projeto forma o nome em `main.tf`:
...
E cria o recurso com:

```hcl
name = var.acr_name
```

O módulo está funcionando como esperado. A correção precisa acontecer na origem `local.acr_name`.

## 57. Correção imediata e isolada

Como `var.suffix` também nomeia AKS, peerings e vínculos DNS, não altere o sufixo global apenas para resolver o ACR.
...
Mantenha a linha do AKS sem alteração:

```hcl
aks_name = "aks-imersao-${var.suffix}"
```

Essa é a menor correção para a fase atual. Uma evolução posterior pode criar uma variável específica `acr_name`, mas isso exige editar `variables.tf`, `terraform.tfvars` e possivelmente o exemplo.

## 58. Invalidar o plano que contém o nome antigo

Preserve o plano incorreto com um nome explícito:

```bash
if [ -f tfplan-novo ]; then
  mv tfplan-novo tfplan-nome-acr-antigo
fi
```
...
```bash
terraform apply tfplan-nome-acr-antigo
```

Ele tentaria novamente criar o nome indisponível.

## 59. Gerar e revisar o plano corrigido

Confirme primeiro a alteração:

```bash
sed -n '1,12p' main.tf
```

Confirme novamente a disponibilidade:
...
terraform fmt -recursive
terraform validate
terraform plan -out=tfplan-acr1965
terraform show tfplan-acr1965
```

Antes de aplicar, procure obrigatoriamente:
...
O número de recursos pode ser maior que um quando AKS ou conectividade privada forem habilitados. Nesta fase exibida, somente o ACR está habilitado para criação.

## 60. Por que o vínculo DNS não apareceu no plano

O vínculo raiz possui:

```hcl
count = var.enable_aks_private_connectivity ? 1 : 0
```

Como não apareceu no plano atual, a condição está falsa nesta fase, ou suas dependências do AKS não estão habilitadas. Isso pode ser esperado no desenho por fases.

Verifique o valor:

```bash
rg -n 'enable_aks_private_connectivity' variables.tf terraform.tfvars main.tf
```

Não habilite essa opção apenas para fazê-la aparecer. Ela depende da VNet do AKS, dos peerings e dos demais recursos da fase correspondente.

## 61. Atenção futura — zona DNS declarada em dois lugares

Foi encontrada outra declaração da mesma zona em:
...
Agora a zona já é gerenciada na raiz por:

```hcl
azurerm_private_dns_zone.portal_sql
```

Se o módulo `sql-database` for habilitado futuramente com `enable_private_endpoint = true`, poderá tentar criar a mesma zona novamente e produzir conflito.

A arquitetura deve ter um único proprietário da zona. A abordagem recomendada é:

1. manter a zona na raiz ou em um módulo central de DNS;
2. passar o ID da zona existente ao módulo SQL;
3. fazer o módulo SQL criar somente o Private Endpoint e o grupo de zona necessário;
4. não declarar novamente `privatelink.database.windows.net` dentro do módulo SQL.

Essa refatoração não é necessária para concluir a criação isolada do ACR, mas deve ser realizada antes de habilitar a fase do Private Endpoint SQL.

## 62. Atualização — plano antigo reaplicado por engano

O plano aplicado ainda mostrava explicitamente:
...
Como esse arquivo de plano continha o nome indisponível, o Azure retornou novamente `409 AlreadyInUse`. O comportamento é esperado e não representa um novo erro de Terraform.

O ACR não foi criado. A zona DNS criada anteriormente continua válida no estado.

## 63. Barreira obrigatória antes de gerar outro plano

Não execute outro `terraform apply` até os três testes abaixo apresentarem o nome novo.

### Teste 1 — conteúdo do arquivo
...
### Teste 2 — valor calculado pelo Terraform

```bash
printf 'local.acr_name\n' | terraform console
```
...
Se retornar `"acrimersaoazureshop"`, a configuração ainda não foi corrigida ou o comando está sendo executado em outro diretório.
...
```text
/home/carlos/azureshop/infra/terraform
```

O caminho real pode variar conforme a pasta pessoal, mas deve terminar em `/azureshop/infra/terraform`.

### Teste 3 — disponibilidade no Azure
...
## 64. Isolar os planos para evitar nova confusão

Liste os planos existentes:

```bash
ls -lh tfplan*
```

Mova os dois planos antigos para nomes claramente inválidos para uso:

```bash
if [ -f tfplan ]; then mv tfplan tfplan.NAO-APLICAR; fi
if [ -f tfplan-novo ]; then mv tfplan-novo tfplan-novo.NAO-APLICAR; fi
```

Não aplique arquivos terminados em `.NAO-APLICAR`.

## 65. Criar um plano com nome inequívoco

Somente depois dos três testes obrigatórios:

```bash
terraform fmt -recursive
terraform validate
terraform plan -out=tfplan-acrimersaoazureshop1965
```

Examine especificamente o nome salvo no plano:

```bash
terraform show -no-color tfplan-acrimersaoazureshop1965 \
  | sed -n '/azurerm_container_registry/,/^    }/p' \
  | rg 'name\s+=' 
```
...
Também faça a revisão completa:

```bash
terraform show tfplan-acrimersaoazureshop1965
```

## 66. Aplicar somente o plano conferido

Se e somente se o plano mostrar o nome novo:

```bash
terraform apply tfplan-acrimersaoazureshop1965
```
